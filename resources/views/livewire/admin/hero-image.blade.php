<div class="card" style="margin-bottom:1.5rem">
    ...
    <div style="padding:1.25rem;display:flex;gap:1.5rem;flex-wrap:wrap;align-items:flex-start">
        ...
        <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:1rem">
            ...
            <form method="POST" action="{{ route('admin.hero-image.upload') }}" enctype="multipart/form-data">
                @csrf
                <div>
                    ...
                    <div class="upload-zone" id="hero-drop-zone"
                        ...
                        <span class="upload-text" id="hero-upload-label">Click or drag image here</span>
                        <span class="upload-subtext">PNG, JPG, WebP · max 10MB</span>
                    </div>
                    <input id="hero-file-input" name="image" type="file" accept="image/*" class="hidden-input"
                        onchange="document.getElementById('hero-upload-label').textContent = this.files[0] ? this.files[0].name : 'Click or drag image here'">
                </div>
            ...
            @if($hasBackup)
                <form method="POST" action="{{ route('admin.hero-image.restore') }}">
                    @csrf
    ...
    <script>
        function handleHeroDrop(event) {
            ...
            document.getElementById('hero-drop-zone').classList.remove('dragging');
            ...
            document.getElementById('hero-upload-label').textContent = file.name;
        }
    </script>
</div>
